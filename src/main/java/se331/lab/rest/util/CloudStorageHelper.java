...
public class CloudStorageHelper {
    ...
    public StorageFileDto uploadSingleImage(MultipartFile file, final String bucket) throws IOException, ServletException {
        ...
        if (fileName != null && !fileName.isEmpty() && fileName.contains(".")) {
            ...
            for (String s : allowedExt) {
                if (extension.equals(s)) {
                    String urlName = this.uploadFile(file, bucket);  // Assuming the uploadFile method exists in this class
                    return StorageFileDto.builder()
                            .name(urlName)
                            .build();
                }
    ...
    static {
        ...
        InputStream serviceAccount =
                null;
        try {
            serviceAccount = new ClassPathResource("imageupload-6ee6b-abc2d605e054.json").getInputStream();
            storage = StorageOptions.newBuilder()
                    .setCredentials(GoogleCredentials.fromStream(serviceAccount))
                    .setProjectId("imageupload-6ee6b")
                    .build().getService();
        } catch (IOException e) {
            e.printStackTrace();
        }
